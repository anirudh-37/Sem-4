...
class BinarySearchTree:
    ...
    """
    def inorderTraverse(self,v,li):
        curnode = v
        if (curnode.leftchild != None):
            self.inorderTraverse(curnode.leftchild,li)
        li.append(v.element)
        if (curnode.rightchild!=None):
            self.inorderTraverse(curnode.rightchild,li)
    ...
    def percentile(self,k):
        c=0
        il = []
        self.inorderTraverse(self.root,il)
        for i in il:
            if i <= k:
                c += 1
        p = math.ceil((c/len(il))*100)
        return p
    def greater(self,k):
        c=0
        il = []
...
def testbst():
    ...
    while inputs>0:
        ...
        if(operation[0]=="H"):
            ...
            print(ds.percentile(int(operation[1])))
